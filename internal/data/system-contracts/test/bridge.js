...
contract('Bridge.sol', (accounts) => {
  ...
  before(async () => {
    ...
    bridge = await Bridge.new([weth.address], {from: USER});
  });
  ...
  describe('depositToken()', async () => {
    it('should deposit erc20 compatible tokens successfully', async () => {
      ...
      const result = await bridge.depositToken(weth.address, odinAddress, depositAmount, {from: USER});

      assert.equal(result.logs.length, 1);
      assert.equal(result.logs[0].event, 'TokenDeposited');
      assert.equal(result.logs[0].args._userAddress, USER);
